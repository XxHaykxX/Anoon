...
## Что дальше (ЖДЁТ данных/действий юзера)

1. 🔹 Включить **Anonymous sign-ins** в Supabase Auth → активирует persist/push/модерацию в рантайме.
2. Применить таблицу **PushSubscription** к живой БД (`pnpm --filter @anoon/db push`) + `migrate resolve --applied 0_init`.
3. Заполнить **admin/.env** + `node --env-file=.env scripts/create-admin.mjs`, затем `NEXT_PUBLIC_DATA_MODE=api`.
4. **R2-ключи** → Фаза E (медиа реально отправляются).
5. **GitHub+Vercel+хостинг** → деплой (CI уже готов).
6. ⚠️ **Ротировать** засвеченные секреты: SUPABASE_SECRET_KEY, service_role, DB-пароль.
