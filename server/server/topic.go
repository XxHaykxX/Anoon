...
// isAnon reports whether this is an "anonymous" topic where peers must not see
// each other's identity (UID, Public/Trusted, or message sender). A topic is
// marked anonymous by the companion service (acting as topic admin/ROOT) via
// {set desc aux:{"anon":true}}; the flag is persisted in the topic's aux map.
// Indexing a nil map returns the zero value, so this is safe on topics without aux.
...
func (t *Topic) prepareBroadcastableMessage(msg *ServerComMessage, uid types.Uid, isChanSub bool) {
	...
	// anoon anon patch (variant E) -- BEGIN
	// Anonymous topic: hide the sender's identity on all broadcastable messages
	// (live {data}, and typing/read receipts in {info}) so peers can't see each
	// other's UID. Presence {pres} is not blanked here: anonymous topics carry no
	// 'P' access bit, so presence is not broadcast to peers (online status is
	// surfaced separately by the companion service).
	if t.isAnon() {
		if msg.Data != nil {
			msg.Data.From = ""
		}
...
func (t *Topic) replyGetSub(sess *Session, asUid types.Uid, authLevel auth.Level, asChan bool, msg *ClientComMessage) error {
	...
	for i := range subs {
		...
		// anoon anon patch (variant E) -- BEGIN
		// Anonymous topic: hide a peer's identity (UID, Public, Trusted) from other
		// subscribers. The requester's own subscription (uid == asUid) is left intact
		// so the client still knows who it is. 'private' is per-user and never a peer's.
