...
func pbCliDeserialize(pkt *pbx.ClientMsg) *ClientComMessage {
	...
	if hi := pkt.GetHi(); hi != nil {
		...
		if sq := set.GetQuery(); sq != nil {
			if q := pbSetQueryDeserialize(sq); q != nil {
				msg.Set.MsgSetQuery = *q
			}
		}
...
func pbSetQueryDeserialize(in *pbx.SetQuery) *MsgSetQuery {
	...
	// Auxiliary data (e.g. the anoon anon flag set over gRPC as {set aux:{anon}}).
	// Without this branch a Set carrying only aux deserializes to nil and the
	// caller's dereference panics (crashing the gRPC server).
