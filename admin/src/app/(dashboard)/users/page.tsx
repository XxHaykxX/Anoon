...
export default function UsersPage() {
  ...
  // Глобальный поиск из шапки приходит через ?q=. Фильтруем локально (список грузится целиком,
  // pagination=off) — одинаково работает и в mock, и в api режиме. #ID — точное совпадение,
  // ник — вхождение (без регистра). Ведущий «#» в запросе игнорируем.
  const searchParams = useSearchParams();
  const q = (searchParams.get("q") ?? "").trim();
  const needle = q.replace(/^#/, "").toLowerCase();
  const allRows = result?.data ?? [];
  const rows = needle
    ? allRows.filter((u) => u.publicId.toLowerCase() === needle || u.nickname.toLowerCase().includes(needle))
    : allRows;
    ...
    <div>
      ...
      {q && (
        <div className="mb-4 flex items-center gap-2 text-sm text-fg-secondary">
          <span>
            Поиск: <span className="font-medium text-fg">{q}</span> · найдено: {rows.length}
          </span>
          <Link
            href="/users"
            className="inline-flex items-center gap-1 rounded-md bg-surface-2 px-2 py-1 text-xs text-fg-secondary transition hover:text-accent"
          >
            <X size={13} /> Сбросить
          </Link>
        </div>
      ...
      {query.isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-surface-1/50 px-4 py-10 text-center text-sm text-fg-muted">
