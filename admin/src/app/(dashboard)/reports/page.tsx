...
export default function ReportsPage() {
  ...
  // всегда указывает на валидный индекс (для J/K-навигации), поэтому нужен отдельный флаг —
  // в отличие от Чаты, где sel начинается с null и сам по себе годится для переключения панелей.
    ...
    <div>
      ...
      {query.isLoading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
          {/* Левая панель — список. На мобиле прячется, когда открыты детали жалобы. */}
              ...
              <AnimatePresence initial={false}>
                {rows.map((r, i) => (
                  <motion.button
                    ...
                    aria-current={i === sel}
                    className={cn(
                      // flex-wrap: на узких экранах бейджи (не сжимаются, whitespace-nowrap) не влезают
                      // в одну строку с ником — переносим вместо обрезки по краю карточки.
                      "flex w-full flex-wrap items-center gap-2 gap-y-1.5 rounded-xl border bg-surface-1 p-3 text-left transition-colors sm:gap-3",
          ...
          {/* Правая панель — детали выбранной жалобы. На мобиле видна только после выбора жалобы. */}
          <div className={cn("lg:sticky lg:top-4 lg:self-start", !mobileOpen && "hidden lg:block")}>
            {selected ? (
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="-ml-1.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-fg-secondary hover:bg-surface-2 lg:hidden"
                      >
                        <ArrowLeft size={16} />
                      </button>
