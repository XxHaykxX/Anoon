...
// Глобальный поиск в шапке: ник или #ID. Submit (Enter) уводит на /users?q=…,
// где список фильтруется локально. Значение синхронизируется с ?q= в URL.
function HeaderSearch() {
  ...
  // Синхронизация с URL (напр. после «Сбросить» на странице пользователей) без setState-в-effect.
  if (urlQ !== prevUrlQ) {
    setPrevUrlQ(urlQ);
    setValue(urlQ);
  }
  ...
  return (
    <form
      onSubmit={submit}
      role="search"
      className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-border bg-surface-2 px-3 py-1.5 text-sm text-fg-muted focus-within:border-accent"
    >
      <Search size={16} className="shrink-0" />
      <input
        value={value}
        onChange={(e) => setValue(e.target.value)}
        placeholder="Поиск: ник или #ID"
        aria-label="Поиск: ник или #ID"
      ...
      {value && (
        <button
          ...
          className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-fg-muted transition hover:text-fg"
        >
          <X size={15} />
        </button>
      )}
    ...
    <div className="flex min-h-dvh bg-bg text-fg">
      ...
      <aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-surface-1 md:flex">
        <div className="flex items-center gap-2.5 px-5 py-5">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent text-accent-fg font-semibold">b</div>
