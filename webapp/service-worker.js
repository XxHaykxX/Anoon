...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async _ => {
    ...
    // App-shell (navigations + html/css/js): NETWORK-FIRST so code updates are
    // never masked by a stale cached copy. Fall back to cache only when offline.
    const isAppShell = event.request.mode == 'navigate' ||
      /\.(?:html|css|js)$/i.test(reqUrl.pathname);
    if (isAppShell) {
      try {
    ...
    // Other assets (images/docs): cache-first, ignoring auth query signatures.
