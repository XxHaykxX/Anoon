...
> **СВЕЖЕЕ (2026-08-06) — читать первым:**
> - [SESSION-2026-08-06.md](SESSION-2026-08-06.md) — волна безопасности:
>   закрыто всё из SECURITY-FINDINGS плюс аудит авторизации (7 находок) и
>   четыре случая, где клиент утверждал несуществующее. Миграции 0010–0013.
>   **Раздел §4 «Что осталось» — там всё незакрытое**, включая 7 упавших
>   real-E2E, которые НЕ разобраны.
> - [qa/](qa/) — чеклисты и материалы той волны: QA-SECURITY-WAVE (проверка
>   каждой находки + регресс), QA-ADMIN, SCREEN-MAP (пути клика до всех
>   экранов), AUTHZ-SWEEP (аудит с таблицей покрытия), UI-FIXLIST.
> - [SECURITY-FINDINGS.md](SECURITY-FINDINGS.md) — исходный отчёт. **Все
>   находки закрыты 2026-08-06**, читать как историю, а не как список дел.
> - [SESSION-2026-08-05.md](SESSION-2026-08-05.md) — объединение трёх папок в одну,
>   переименование, ~15 починенных багов.
>
> Пути в тексте ниже могли устареть: весь код теперь в `Desktop/anoon`
> (`frontend/`, `server/`, `server-stack/`, `webapp/`, `admin/`, `docs/`),
> приложение открывается по `/anoon`, а не `/badu`.
...
**Актуальный список незакрытого — `docs/SESSION-2026-08-10.md`, разделы 2 и 3.**
Ниже — сжатая выжимка на 2026-08-10.

1. **Ждёт владельца:** платёжный провайдер (вопрос открыт его решением —
   критический путь резидентное армянское юрлицо, а не код); SMTP (отправка
   заглушена, код подтверждения идёт в лог); Q7a (чтение переписки модератором);
   подтвердить жёлтый `themeColor` над чёрным приложением.
2. **На нас, не сделано:** живой вход через Google человеком не проверялся;
   UI-правки не выкачены на прод (прод `ffa095c`, main `78ff670`); восстановление
   из бэкапа ни разу не проверено; внешний монитор `/api/health` не заведён;
   аудит мёртвых кнопок не доведён.
3. Горизонтальное масштабирование — `SCALING-PLAN.md`, не начато, до трафика не нужно.
4. Мелкий тех-долг: guard `startCompanionEvents` до sessionToken, `age?:number` в
   BasicSignInInput.

**Снято владельцем из планов:** собственный домен вместо sslip.io; платный Supabase.
